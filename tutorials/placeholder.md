...
like if user A is `Abdi`\
bot should say: `Hi @Abdi`

if user A became `ButterFly`\
...
so in the message content and any [Text](../inputs/text.md) input, you can use this placeholder this way:
```
<place holder name>
```
...
each placeholder is of specific type, like `member` is of type [`Member`](../placeholders/member.md)\
and this placeholder type provides some useful properties not just the mention of user\
